...
function setVelocity(particleData, positions, index) {
    if (particlePositions[index + 1] < -halfRadius || particlePositions[index + 1] > halfRadius)
        particleData.velocity.y = -particleData.velocity.y;

    if (particlePositions[index] < -halfRadius || particlePositions[index] > halfRadius)
        particleData.velocity.x = -particleData.velocity.x;

    if (particlePositions[index + 2] < -halfRadius || particlePositions[index + 2] > halfRadius)
...
function getHelperGrid() {
    helperGrid = new THREE.BoxHelper(new THREE.Mesh(new THREE.BoxGeometry(radius, radius, radius)));
    helperGrid.material.color.setHex(0x474747);
    helperGrid.material.blending = THREE.AdditiveBlending;
