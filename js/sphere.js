...
function initGUI() {
    ...
    motionFolder.add(effectController, 'enableTouchRotation').name('Touch Rotation').onChange((value) => {
...
function updateParticleVelocity() {
    for (let i = 0; i < particleCount; i++) {
        ...
        const distanceFromCenter = particlePosition.length();
        if (Math.abs(distanceFromCenter - halfRadius) < 0.01) {
            velocity.set(0, 0, 0);
            continue;
        }
        ...
        if (distanceFromCenter > halfRadius) {
            particlePosition.normalize().multiplyScalar(halfRadius);
            velocity.negate();
        } else {
...
function animate() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        const distance = calculateDistance(particlePositions, baseIndex1);
...
function createParticleData() {
    ...
    const position = new THREE.Vector3(
        halfRadius * Math.sin(phi) * Math.cos(theta),
        halfRadius * Math.sin(phi) * Math.sin(theta),
        halfRadius * Math.cos(phi)
    );
...
function calculateDistance(positions, index) {
...
function getHelperGrid() {
    helperGrid = new THREE.LineSegments(
        new THREE.WireframeGeometry(new THREE.SphereGeometry(halfRadius, 32, 32)),
...
function initOrbitControls() {
    ...
    orbitControls.enabled = effectController.enableTouchRotation;
}
